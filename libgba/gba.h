...
struct Sound {
  Sound(u16_t* song, u16_t song_size, u16_t tempo = 6)
    : _song_ch1(song)
    , _song_size(song_size)
    , _tempo(tempo)
  {}

  Sound(u16_t* song_ch1, u16_t* song_ch2, u16_t song_size, u16_t tempo = 6)
    : _song_ch1(song_ch1)
    , _song_ch2(song_ch2)
    , _song_size(song_size)
    , _tempo(tempo)
  ...
  static constexpr u16_t TypeA = 
    ...
    (0b10     <<  6) | // Wave pattern duty (2 = 50% = normal)
    (0b010    <<  8) | // Envelope Step-Time; units of n/64s
  ...
  static constexpr u16_t TypeD = 
    (0b111000 <<  0) | // Sound length; units of (64-n)/256s  (0-63)
    (0b00     <<  6) | // Wave pattern duty (1 = 25%)
    (0b101    <<  8) | // Envelope Step-Time; units of n/64s
    (0b1      << 11)   // Envelope Direction
  ;
